...
from libcamera import controls  # para autofocus v3 (con fallbacks abajo)
...
def main():
    ...
    # --------- AUTOFOKUS cámara v3 (robusto a versiones) ----------
    # 1) Modo AF continuo + velocidad (si tu build lo soporta)
    try:
        picam2.set_controls({
            "AfMode":  controls.AfModeEnum.Continuous,
            "AfSpeed": controls.AfSpeedEnum.Fast,   # pon Normal si ves mucho “hunting”
        })
    except Exception:
        pass  # por si tu libcamera no expone estos enums

    # 2) Rango de enfoque (opcional; no todas las builds lo tienen)
    try:
        picam2.set_controls({"AfRange": controls.AfRangeEnum.Normal})  # o Full
    except Exception:
        pass
    ...
    # 4) Trigger de enfoque al arrancar:
    #    usa el enum si existe; si no, usa 0 (Start) que funciona en builds antiguas.
    try:
        af_start = controls.AfTrigger.Start
    except Exception:
        af_start = 0  # 0 = Start; 1 = Cancel en builds viejas
    try:
        picam2.set_controls({"AfTrigger": af_start})
    except Exception:
        pass
    # ---------------------------------------------------------------
